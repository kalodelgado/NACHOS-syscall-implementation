#NachOS System Call Implementation


The Implementation of system calls started with appending Exception Handler and understanding various parts of the code about their functionality and their utility in doing what they do.
...
4. In this new constructor, the Page Table mapped virtual address to physical address with a function virtual address + totalPagesCount(Where totalPagesCount is a global which stores the total number of physical pages which have been alloted as of yet.)
    
5. Next we duplicated the parent's physical memory exactly into the child's physical memory location, a simple copy in machine->mainMemory

6. Now the return value of child was changed to zero and it's state is saved (inspired from NachOSThread::SaveUserState)

7. The parent's return address is set to the child's pid and the child's stack is set to the function 'myFunction' which is defined in thread.h. This essentially mimics the behaviour a normal thread has on returning from a sleep. This is done to ensure that a newly forked thread behaves like any other woken thread.
...
1. For each thread we have defined an extra field of integer type which is incremented every time we read an instruction, which is to be executed.

2. This field is incremented in oneInstruction function in mipsim.c